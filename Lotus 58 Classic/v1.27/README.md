...
The latest version of the classic TRRS version uses winged SK6812mini-E RGB for BOTH per-key and under-glow RGB for an easier build. Revamped RGB selection allows of BOTH underglow and per-key RGB to be soldered in, and selectable, but without exceeding the power avaiable form the USB port. Retained support for battery cut-off switch for easy wireless builds with nice!nano. Several minor tweaks.

Fully backwards compatible, physically identical outline for plates etc. and identical firmware.
